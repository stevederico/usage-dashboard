...
impl Db {
    ...
    ///
    /// Cursor's `state.vscdb` is another app's file. Read-write open would
    /// create a missing path and take a writer lock while Cursor is running.
    ///
    /// # Errors
    /// Returns [`DbError`] when the path is missing, not a database, or contains a NUL.
    pub fn open_readonly(path: &str) -> Result<Db, DbError> {
        let flags = ffi::SQLITE_OPEN_READONLY | ffi::SQLITE_OPEN_FULLMUTEX;
        let db = Self::open_with(path, flags)?;
        db.exec("PRAGMA query_only = ON")?;
        Ok(db)
    }

    /// Open `path` with the given `sqlite3_open_v2` flags.
    fn open_with(path: &str, flags: c_int) -> Result<Db, DbError> {
